...
(define (interp expr f-list)
  (match expr
    [(num n) n]
    [(add l-expr r-expr) (+ (interp l-expr f-list) (interp r-expr f-list))]
    [(sub l-expr r-expr) (- (interp l-expr f-list) (interp r-expr f-list))]
    [(if0 c-expr t-expr f-expr) (if (zero? (interp c-expr f-list))
                                    (interp t-expr f-list)
                                    (interp f-expr f-list))]
    [(with x named-expr body)
